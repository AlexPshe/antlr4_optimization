...
from src.antlr4.error.Errors import IllegalStateException
...
cdef class BufferedTokenStream(TokenStream):
    ...
    #/
    def sync(self, int i):
        cdef int n = i - len(self.tokens) + 1 # how many more elements we need?
        if n <= 0 :
            return True
        cdef int fetched = self.fetch(n)
        return fetched >= n
    ...
    def getTokens(self, int start, int stop, types:set=None):
    ...
    cdef int adjustSeekIndex(self, int i):
        return i
    ...
    #/
    def nextTokenOnChannel(self, int i, int channel):
        self.sync(i)
    ...
    def previousTokenOnChannel(self, int i, int channel):
    ...
    def getHiddenTokensToRight(self, int tokenIndex, int channel = -1):
        self.lazyInit()
    ...
    def getHiddenTokensToLeft(self, int tokenIndex, int channel = -1):
        self.lazyInit()
        if tokenIndex<0 or tokenIndex>=len(self.tokens):
            raise Exception(str(tokenIndex) + " not in 0.." + str(len(self.tokens)-1))
        from Lexer import Lexer
        ...
        cdef int from_ = prevOnChannel+1
        to = tokenIndex-1
        return self.filterForChannel(from_, to, channel)


    def filterForChannel(self, int left, int right, int channel):
        hidden = []
        cdef int i
    ...
    cdef str getSourceName(self):
        return self.tokenSource.getSourceName()

    # Get the text of all tokens in this buffer.#/
    cdef str getText(self, int start, int stop):
        self.lazyInit()
        self.fill()
